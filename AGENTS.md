...
- The budget can ask instead of degrading, through the `budget` permission: `allow` degrades as before, `ask` stops at each multiple of the budget and lets the person decide, `deny` ends the run at the crossing. An authorized run keeps its model and tools — degrading a run somebody just paid to continue answers a question nobody asked — and a refused one ends in the same text-only summary turn as the ceiling, carrying the reason they gave.
