...
- `budget` — triggered when an agent's spend crosses its [`budget`](/docs/agents#budget); `"allow"` lets it carry on cheaply, `"ask"` stops to ask you, `"deny"` ends the run there
