...
- `budget` — triggered while an agent is past its [`budget`](/docs/agents#budget); `"allow"` lets it carry on cheaply, `"ask"` asks you once per request, `"deny"` ends every request that goes past it. Unlike the others, only a rule naming `budget` applies: a catch-all is about tools, and reading it as a spending checkpoint would end runs nobody configured one for
...
- `budget` defaults to `"allow"`, so a budget makes an agent cheaper rather than stopping it.
