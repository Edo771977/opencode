...
:::note
Spend is measured from the usage each provider reports. A local model that reports no cost never reaches a budget.
:::
...
2. Otherwise the cheapest recent model of the session's provider that can call tools.
...
- A model that cannot call tools is never selected, because an agent's turn carries tool definitions. A `small_model` that cannot is skipped outright rather than falling through to step 2, and a warning is logged — the point of naming a model is not to get a different one.
...
- A variant is never inherited and always honored when written: the session's variant is meaningless on a different model, so it is not carried over, while a variant written into `small_model` names that exact pair. With none written, the small model's own default variant applies.
- A value that is not a `provider/model` reference, or that names a variant the model does not offer, is ignored with a warning rather than silently reshaping the request.
