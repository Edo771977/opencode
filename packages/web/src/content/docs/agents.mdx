...
- The session's variant is never carried over. A variant is chosen for one model, and the same name on another is a different setting or none at all, so a turn moved to the small model is sent without it.
