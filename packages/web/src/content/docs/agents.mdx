...
- The session's variant is never carried over, and a variant written into `small_model` always is. A variant is chosen for one model, and the same name on another is a different setting or none at all — so the turn is sent with the variant written next to the model it moved to, and with nothing otherwise.
