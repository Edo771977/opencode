...
`budget_stop` is the ceiling, and it does stop: the agent gets one text-only turn to summarize what it did and what is left. It is off unless you set it. It suits a subagent, whose caller reads the partial result and decides what to do next — retry it differently, split the work, or do it itself. On a primary agent it ends the run and waits for you, so set it only if that is what you want.

Both are counted per agent per session, so switching agents mid-session gives each its own budget, and a subagent's session carries only its own work. An agent with no `budget` is never degraded.

Set `budget_stop` above `budget` if you want both to happen. They are independent thresholds, so a ceiling at or below the budget stops the agent at a point it would never have reached cheaply — which is a reasonable thing to ask for, but not usually what someone who set both had in mind.
