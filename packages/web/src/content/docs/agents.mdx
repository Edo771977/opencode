...
1. The [`small_model`](/docs/config#models) option, if you set one and it is in your catalog. It takes the same `provider/model` form as [`model`](#model).
...
- A variant is never inherited: the session's variant is meaningless on a different model, so the small model runs with its own default. A variant written into `small_model` selects the model it names and nothing more.
