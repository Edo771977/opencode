...
The `small_model` option configures a separate model for lightweight tasks like title generation, and for any agent that sets [`small`](/docs/agents#small-model). By default, OpenCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model. A model that cannot call tools is never used for an agent turn.
